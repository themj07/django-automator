...
export async function askGeminiForModels(promptText) {
    if (!promptText) throw new Error('Prompt vide');

    const response = await client.models.generateContent({
        model: 'gemini-2.5-flash', // Use a fast, capable model
        contents: [
            { role: 'user', parts: [{ text: `System Instruction: ${SYSTEM_PROMPT}\n\nTask: Génère la définition JSON des modèles Django pour : ${promptText}` }] }
        ],
        config: {
            // Force the output to be JSON
            responseMimeType: "application/json",
            temperature: 0.0,
            systemInstruction: SYSTEM_PROMPT, // System prompt can be passed here
        }
    });

    const raw = response.text; // The text property contains the JSON string
    if (!raw) throw new Error("Réponse vide de l'IA");

    // The Gemini API should return clean JSON if responseMimeType is set, 
    // but we keep the robust extraction logic as a safeguard.
